        ...
        try {
            ...
            if ("6240".equals(info.getCode())) {
                ...
                if(errorInfo.getMessage().contains("Duplicate Name Exists Error")){
                    errorInfo.setMessage("customer name or email already exists on quickbooks");
                }
...
//      customerRequest.setPrimaryEmailAddr(primaryEmailAddr);
        ...
        createQueue.setCustomerTypeId(quickBookDTO.getCustomerTypeId());
        createQueue.setCustomerTypeName(quickBookDTO.getCustomerTypeName());
